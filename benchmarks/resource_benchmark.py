"""Measure OpenBook's practical resource footprint with no external services.

The benchmark intentionally uses only the Python standard library so it can run
in CI and on fresh developer machines without API keys. It exercises a local
SQLite-backed memory store as a portable baseline for footprint questions:
database size, insert latency, search latency, context pack size, and process
memory when the platform exposes it.
"""
...
def render_summary(results: dict[str, object]) -> str:
    ...
    process = results["process_memory"]
